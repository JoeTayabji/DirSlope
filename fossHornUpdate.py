...
shp_out_name = input("Output filename (do not include .shp extension): ") + ".shp" # output shapefile name
...
if geometry_type == 4:
    ...
    line_build_path = os.path.join(out_dir, 'temp_line_build.shp')
    line_build_ds = ogr.GetDriverByName("ESRI Shapefile").CreateDataSource(line_build_path)
    line_build_layer = line_build_ds.CreateLayer("Dir Slope Line", geom_type = ogr.wkbLineString, srs = line_in_spref)
    ...
    for i in range(num_fields):
        ...
        line_build_layer.CreateField(new_field)
    '''
    temp_out_name = "temp_name_for_ds" + ".shp" # output shapefile name
    temp_out_path = os.path.join(out_dir, temp_out_name)
    temp_ds = ogr.GetDriverByName("ESRI Shapefile").CreateDataSource(line_out_path)
    temp_layer = temp_ds.CreateLayer("Dir Slope Line", geom_type = ogr.wkbLineStringZM, srs = line_in_spref)
    pt_count = 0
    previous_x, previous_y = None, None
    for p in line_in_layer:
            geom = p.GetGeometryRef()
            point = geom.GetGeometryRef(0)
            x, y, _ = point.GetPoint(0)
            if pt_count != 0:
                # add line to layer
                temp_geom = ogr.Geometry(ogr.wkbLineString)
                temp_geom.AddPoint(previous_x, previous_y)
                temp_geom.AddPoint(x, y)
                temp_feat = ogr.Feature(temp_layer.GetLayerDefn())
                temp_feat.SetGeometry(temp_geom)
                temp_layer.CreateFeature(temp_feat)
            previous_x, previous_y = x, y
            pt_count += 1
    #temp_ds = None
    #line_in_layer = None
    line_in_layer = temp_layer
    #temp_layer = None 
...
if osr.SpatialReference(dem_spref).GetAttrValue("PROJCS") == line_in_spref.GetAttrValue("PROJCS"):
    print("DEM and input line have the same projection: {0}.".format(osr.SpatialReference(dem_spref).GetAttrValue("PROJCS")))
else:
...
'''
sample_interval = 10 # resampling distance
#line_in_layer.ResetReading()
print(type(line_in_layer))
counter = 0
for p in line_in_layer:
    print(counter)
