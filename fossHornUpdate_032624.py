...
out_dir = r"E:\DIRECTIONAL_SLOPE\WorkingScripts\SampleData"
os.makedirs(out_dir, exist_ok=True) # Create the output directory if it doesn't exist
line_in_path = r"E:\DIRECTIONAL_SLOPE\WorkingScripts\SampleData\FishHatchLine_UTM13_Subset.shp"
#line_in_path = r"C:\Users\jtaya\OneDrive\Documents\DirSlope\q\fishhatchinputs\multipoints.shp"
shp_out_name = "FishHatchLine_FOSS_DS_Samp0.5m_032624.shp" # output shapefile name
...
# Get EPSG codes
dem_epsg = osr.SpatialReference(dem_spref)
#.GetAuthorityCode(None)
#print (line_in_spref)
line_in_epsg = line_in_spref.GetAuthorityCode(None)
print (dem_epsg, line_in_epsg)

# Check if EPSG codes are the same
if dem_epsg == line_in_epsg:
    print("DEM and input line have the same projection: {0}.".format(dem_epsg))
else:
...
def dir_slope(x, y, previous_x, previous_y):
    ...
    bearList.append(degrees(bearing))
    dSlopeList.append(dir_slope)
...
for p in line_in_layer:
    ...
    while distance < line_length:
        ...
        # Add each x,y to Point List to split the input line by
    ...
    ##Segmentize!!
    shapely_line2D=shapely.ops.transform(lambda x, y, z=None: (x, y), shapely_line)
    print (segmentize(shapely_line2D, max_segment_length=0.5))
    
    for point in pointList:
        ##print(point)
        point2=Point(point)
        print(shapely_line2D.intersects(point2))
        ##print (point2)
        point3=snap(point2, shapely_line2D,500000)
        print(point3.intersects(shapely_line2D))
        pointList2.append(point3)
    ##print(pointList2)
    points=MultiPoint(pointList2)
    print(points)
    
    result = split(shapely_line2D, points)
    print(result)
    result_line=MultiLineString(result.geoms)
    
    driver = ogr.GetDriverByName('Esri Shapefile')
    ds = driver.CreateDataSource(os.path.join(out_dir,'testSplit.shp'))
    layer = ds.CreateLayer('', geom_type=ogr.wkbLineString, srs = line_in_spref)
    # Add one attribute
    ...
    geom = ogr.CreateGeometryFromWkb(result.wkb)
    ...
    for i in result_line:
        ...
        line_out_feat.SetField("Bearing", bearList[counter])
        line_out_feat.SetField("Dir_Slope", dSlopeList[counter])
...
##    line_out_layer.CreateFeature(line_out_feat)
    #print ("split result-line-length:"+str(result_line.length))
