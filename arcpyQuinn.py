...
with arcpy.EnvManager(outputCoordinateSystem=sr, cellSize=inDEM, snapRaster=inDEM, extent=inDEM,
                      mask=inDEM):
...
for nRow in range (nRows):
    for nCol in range (nCols):
        ...
        ##else: i = fDirs.index(fdirPixel)
            ...
            #i is the index of the pixel in the possible fdirs (1,2,4,8,16,32,64,128)
            i = fDirs.index(fdirPixel)

        # Get the column of the cell to compare
        #Getting the number of the column, and adding dcol at index i from fdirs
        nC = nCol + dCol[i]
